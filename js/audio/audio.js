...
function encodeAudioBufferLame( audioData ) {
	return new Promise( (resolve, reject) => {
		var worker = new Worker('../js/audio/worker/worker.js');
...
function loadAudio4(){
	if (ischangeUpload){
		...
		$('#modalUploadVoice').modal('hide')
		$('#modalMainVoice').modal('show')
	} else {
		alert('ye chizi befrest')
	}
}
function loadAudio3(){
	loadAudio()
	$('#modalRecordVoice').modal('hide')
	$('#modalMainVoice').modal('show')
}
function loadAudio2(){
	...
	fileeeeeeeeeeee = element.files[0]
	ischangeUpload = true
}
function loadAudio() {
	// var element = document.getElementById("audio-file");
