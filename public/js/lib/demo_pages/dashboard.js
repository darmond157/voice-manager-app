...
var Dashboard = function () {
    ...
    var _BarChartDaily = async function (element, barQty, height, animate, easing, duration, delay, color, tooltip) {
        if (typeof d3 == 'undefined') {
            console.warn('Warning - d3.min.js is not loaded.');
            return;
        }
        ...
        // Initialize chart only if element exsists in the DOM
        if ($(element).length > 0) {
            let reports = (shops.report_chart.getReport()).body;
            let reportsResponse = (await _post(reports)).result;
            let list=reportsResponse[0].daily_report;
            ...
            for (var i = 0; i < barQty; i++) {
                amount = list[i]
                bardata.push(amount.data?amount.data.length:0);
            }
            ...
            if (tooltip == 'dailysale') {
                tip.html(function (d, i) {
                    return '<div class="text-center">' +
                        '<h6 class="m-0">' + d + '' + '</h6>' +
                        '<span class="font-size-sm">تعداد فروش</span>' +
                        '<div class="font-size-sm">' + i + ':00' + '</div>' +
                        '</div>'
                });
            }
    ...
    return {
        ...
        initCharts: function() {
            ...
            _chartSparkline('#server-load2', 'area', 30, 50, 'basis', 750, 2000, 'rgba(255,255,255,0.5)');
            _chartSparkline('#server-load3', 'area', 30, 50, 'basis', 750, 2000, 'rgba(255,255,255,0.5)');

            _chartSparkline('#server-load4', 'area', 30, 50, 'basis', 750, 2000, 'rgba(255,255,255,0.5)');
            ...
            // _BarChartDaily('#daily-sale', 24, 50, true, 'elastic', 1200, 50, 'rgba(255,255,255,0.5)', 'dailysale');
            // _BarChart('#members-online-2', 24, 50, true, 'elastic', 1200, 50, 'rgba(255,255,255,0.5)', 'members');
